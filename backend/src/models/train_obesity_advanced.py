...
print("="*70)
print("HEALTHSCOPE - ADVANCED MODEL TRAINING: OBESITY")
print("="*70)
...
categorical_cols_to_drop = ['gender', 'family_history_with_overweight', 'favc', 'caec', 'smoke', 'scc', 'calc', 'mtrans']
X = X.drop(columns=categorical_cols_to_drop, errors='ignore')
...
rf_train_roc = roc_auc_score(y_train_bin, rf_train_proba, average='weighted', multi_class='ovr')
...
rf_test_roc = roc_auc_score(y_test_bin, rf_test_proba, average='weighted', multi_class='ovr')
...
xgb_train_roc = roc_auc_score(y_train_bin, xgb_train_proba, average='weighted', multi_class='ovr')
...
xgb_test_roc = roc_auc_score(y_test_bin, xgb_test_proba, average='weighted', multi_class='ovr')
...
# Random Forest CV
rf_cv_scores = cross_val_score(rf_model, X_train, y_train, cv=5, scoring='roc_auc_ovr_weighted', n_jobs=-1)
print(f"\nRandom Forest CV ROC-AUC: {rf_cv_scores.mean():.4f} (+/- {rf_cv_scores.std():.4f})")

# XGBoost CV
xgb_cv_scores = cross_val_score(xgb_model, X_train, y_train, cv=5, scoring='roc_auc_ovr_weighted', n_jobs=-1)
...
baseline_results = pd.read_csv('models_saved/obesity_baseline_results.csv')
...
if best_model_name == 'Random Forest':
    ...
    # Load baseline model
    best_model = joblib.load('models_saved/obesity_baseline_lr.pkl')
...
# Save best model
best_model_path = 'models_saved/obesity_model.pkl'
joblib.dump(best_model, best_model_path)
print(f"✓ Best model saved to: {best_model_path}")

# Save label encoder
le_path = 'models_saved/obesity_label_encoder.pkl'
joblib.dump(le, le_path)
print(f"✓ Label encoder saved to: {le_path}")

# Save Random Forest
rf_path = 'models_saved/obesity_rf.pkl'
joblib.dump(rf_model, rf_path)
print(f"✓ Random Forest saved to: {rf_path}")

# Save XGBoost
xgb_path = 'models_saved/obesity_xgb.pkl'
...
results_path = 'models_saved/obesity_all_models_results.csv'
...
# ============================================================================
print("\n" + "="*70)
print("ADVANCED MODEL TRAINING SUMMARY - OBESITY")
print("="*70)
print(f"Best Model: {best_model_name}")
print(f"Best ROC-AUC: {best_roc_auc:.4f}")
print(f"Classes: {len(le.classes_)}")
print(f"\nAll models saved to: models_saved/")
print(f"  - obesity_model.pkl (best model)")
print(f"  - obesity_baseline_lr.pkl")
print(f"  - obesity_rf.pkl")
print(f"  - obesity_xgb.pkl")
print(f"  - obesity_label_encoder.pkl")
print("="*70)

print("\n✓ Obesity advanced model training complete!")
